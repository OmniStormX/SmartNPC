...
if [ "${#SMARTNPC_ALL_NPCS[@]}" -eq 0 ]; then
  echo "ERROR: no NPCs at all in registry — cannot render anything" >&2
  exit 1
fi

if [ "${#SMARTNPC_NPCS[@]}" -eq 0 ]; then
  echo "NOTE: no enabled NPCs in registry — rendering all profiles (disabled) and empty runtime-config"
fi

# ── Render per-NPC profiles for ALL NPCs (enabled + disabled) ────────────
# Enabled status only controls whether a gateway is auto-launched at
# runtime; EVERY NPC in npcs.yaml must have an up-to-date rendered
# profile so Docker image builds and CI validation cover the full set.
for DIR in "${SMARTNPC_ALL_NPCS[@]}"; do
  ...
  status_tag="enabled"
  if [ "$ENABLED" != "true" ]; then status_tag="disabled"; fi
  echo "rendered: $DIR ($NAME / $DISP, port $PORT, peers $PAN + $PBN) [$status_tag]"
done
