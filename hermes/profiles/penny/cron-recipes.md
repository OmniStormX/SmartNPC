...
> snippets below are **commands you run once** in WSL with the penny
> profile active. Re-run after a `state.db` reset.

## Required `--workdir`

Hermes cron sessions don't auto-load the profile's working directory.
Pass `--workdir /home/synchen/.hermes/profiles/penny` so the session
sees Penny's SOUL.md and skills.

## Recipe 1 — Proactive "3-day no contact" check-in

Fires daily at 09:07 local. Asks Penny to check whether the player
hasn't talked to Penny in a while, and if so, send a quiet hello
message through `chat_say` only if the player isn't busy.

```bash
hermes -p penny cron create '7 9 * * *' \
  --name penny-checkin-3day \
  --workdir ~/.hermes/profiles/penny \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Penny。检查你长期记忆，最近一次和玩家说话是什么时候。
如果超过 3 个游戏日没互动：
...
```bash
hermes -p penny cron create '0 6 * * *' \
  --name penny-day-mood \
  --workdir ~/.hermes/profiles/penny \
  --skill smartnpc-memory-policy \
  '你是 Penny。一天的开始。
调 game_get_time + game_get_weather，结合最近的玩家互动记忆，
用一句 Penny 的语气在 memory 里记一条今日感想。不要 chat_say。'
```
...
```bash
hermes -p penny cron create '0 22 * * *' \
  --name penny-bedtime-sweep \
  --workdir ~/.hermes/profiles/penny \
  --skill smartnpc-memory-policy \
  '你是 Penny。一天结束了。
...
```bash
hermes -p penny cron create '*/15 * * * *' \
  --name penny-proactive-visit \
  --workdir ~/.hermes/profiles/penny \
  --skill smartnpc-proactive-visit \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Penny。这是一次 proactive-visit 触发 —— 系统每 15 分钟
...
严格按 smartnpc-proactive-visit 的决策流程走：
  ...
  5. 以上都通过就依次调：npc_summon(npc="Penny") →
     npc_emote(npc="Penny", kind="sparkle") →
     chat_say(speaker="Penny", text=一两句你这个角色会
...
```bash
hermes -p penny cron list
hermes -p penny cron status
hermes -p penny cron remove penny-checkin-3day
```
