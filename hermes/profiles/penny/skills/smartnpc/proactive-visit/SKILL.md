...
1. **`npc_summon` yourself**: `npc_summon(npc="Penny")`. The
...
2. **`npc_emote` sparkle bubble**: `npc_emote(npc="Penny",
...
3. **`chat_say` a short in-character opener**: one to two sentences,
   ...
   `speaker`: `Penny`. No `channel` / `group_id` — proactive
