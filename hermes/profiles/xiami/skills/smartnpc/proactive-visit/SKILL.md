...
1. **`npc_summon` yourself**: `npc_summon(npc="XiaMi")`. The
...
2. **`npc_emote` sparkle bubble**: `npc_emote(npc="XiaMi",
...
3. **`chat_say` a short in-character opener**: one to two sentences,
   ...
   `speaker`: `XiaMi`. No `channel` / `group_id` — proactive
