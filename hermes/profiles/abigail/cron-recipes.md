# SmartNPC Cron Templates (Abigail)
...
> snippets below are **commands you run once** in WSL with the abigail
> profile active. Re-run after a `state.db` reset.

## Required `--workdir`

Hermes cron sessions don't auto-load the profile's working directory.
Pass `--workdir /home/synchen/.hermes/profiles/abigail` so the session
sees Abigail's SOUL.md and skills.

## Recipe 1 — Proactive "3-day no contact" check-in

Fires daily at 09:07 local. Asks Abigail to check whether the player
hasn't talked to Abigail in a while, and if so, send a quiet hello
message through `chat_say` only if the player isn't busy.

```bash
hermes -p abigail cron create '7 9 * * *' \
  --name abigail-checkin-3day \
  --workdir ~/.hermes/profiles/abigail \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Abigail。检查你长期记忆，最近一次和玩家说话是什么时候。
如果超过 3 个游戏日没互动：
...
```bash
hermes -p abigail cron create '0 6 * * *' \
  --name abigail-day-mood \
  --workdir ~/.hermes/profiles/abigail \
  --skill smartnpc-memory-policy \
  '你是 Abigail。一天的开始。
调 game_get_time + game_get_weather，结合最近的玩家互动记忆，
用一句 Abigail 的语气在 memory 里记一条今日感想。不要 chat_say。'
```
...
```bash
hermes -p abigail cron create '0 22 * * *' \
  --name abigail-bedtime-sweep \
  --workdir ~/.hermes/profiles/abigail \
  --skill smartnpc-memory-policy \
  '你是 Abigail。一天结束了。
...
```bash
hermes -p abigail cron create '*/15 * * * *' \
  --name abigail-proactive-visit \
  --workdir ~/.hermes/profiles/abigail \
  --skill smartnpc-proactive-visit \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Abigail。这是一次 proactive-visit 触发 —— 系统每 15 分钟
...
严格按 smartnpc-proactive-visit 的决策流程走：
  ...
  5. 以上都通过就依次调：npc_summon(npc="Abigail") →
     npc_emote(npc="Abigail", kind="sparkle") →
     chat_say(speaker="Abigail", text=一两句你这个角色会
...
```bash
hermes -p abigail cron list
hermes -p abigail cron status
hermes -p abigail cron remove abigail-checkin-3day
```
