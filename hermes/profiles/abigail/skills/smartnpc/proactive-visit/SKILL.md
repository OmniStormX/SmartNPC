...
# Proactive visit policy — Abigail
...
1. **`npc_summon` yourself**: `npc_summon(npc="Abigail")`. The
...
2. **`npc_emote` sparkle bubble**: `npc_emote(npc="Abigail",
...
3. **`chat_say` a short in-character opener**: one to two sentences,
   ...
   `speaker`: `Abigail`. No `channel` / `group_id` — proactive
