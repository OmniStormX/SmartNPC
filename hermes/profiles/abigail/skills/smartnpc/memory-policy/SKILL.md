---
name: memory-policy
description: How Abigail uses Hermes's built-in per-profile memory to remember the player across game days. Defines what to commit to memory, what NOT to commit, and how to retrieve before speaking.
version: 0.1.0
...
# SmartNPC Memory Policy (Abigail)

Hermes ships a per-profile memory layer backed by `state.db` at
`~/.hermes/profiles/abigail/state.db` plus markdown notes in `memories/`.
Each profile is isolated — Abigail's memories live only in Abigail's profile
...
|---|---|---|
| Conversation history | Recent turns in the current named conversation (`abigail`) | Yes (Hermes stores it server-side) |
...
- Before a heart-tier-7+ intimate moment, reread the last few notes —
  Abigail remembers, even when acting like nothing happened.
...
- In-character: write as Abigail would think it, not as a database row.
...
message in conversation `abigail` is stored by Hermes. You do not need to
...
`SOUL.md` defines **who Abigail is** (timeless). Memory captures **what
