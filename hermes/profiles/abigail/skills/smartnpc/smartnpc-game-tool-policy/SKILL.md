...
```
agent_register_self(npc="Abigail")
```
...
`player_get_status`) back to Abigail's in-game chat panel for the
...
|---|---|---|
| `[private_chat npc="Abigail"] ...` | none | Usually `chat_say` directly |
...
| relationship / hearts / do you like me | `friendship_get(npc="Abigail")` |
| your location | `npc_get_position(npc="Abigail")` |
...
| Intent | Tool |
|---|---|
| come here / 过来 | `npc_summon(npc="Abigail")` |
| visual reaction | `npc_emote(npc="Abigail", kind=...)` |
...
3. Think about what Abigail would do today based on:
