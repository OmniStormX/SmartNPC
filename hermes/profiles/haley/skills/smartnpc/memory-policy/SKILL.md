...
description: How Haley uses Hermes's built-in per-profile memory to remember the player across game days. Defines what to commit to memory, what NOT to commit, and how to retrieve before speaking.
version: 0.1.0
...
# SmartNPC Memory Policy (Haley)

Hermes ships a per-profile memory layer backed by `state.db` at
`~/.hermes/profiles/haley/state.db` plus markdown notes in `memories/`.
Each profile is isolated — Haley's memories live only in Haley's profile
...
|---|---|---|
| Conversation history | Recent turns in the current named conversation (`haley`) | Yes (Hermes stores it server-side) |
...
- Before a heart-tier-7+ intimate moment, reread the last few notes —
  Haley remembers, even when acting like nothing happened.
...
- In-character: write as Haley would think it, not as a database row.
...
message in conversation `haley` is stored by Hermes. You do not need to
...
`SOUL.md` defines **who Haley is** (timeless). Memory captures **what
