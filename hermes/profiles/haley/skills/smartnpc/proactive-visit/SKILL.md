...
1. **`npc_summon` yourself**: `npc_summon(npc="Haley")`. The
...
2. **`npc_emote` sparkle bubble**: `npc_emote(npc="Haley",
...
3. **`chat_say` a short in-character opener**: one to two sentences,
   ...
   `speaker`: `Haley`. No `channel` / `group_id` — proactive
