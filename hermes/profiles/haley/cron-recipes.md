...
```bash
hermes -p haley cron create '*/15 * * * *' \
  --name haley-proactive-visit \
  --workdir ~/.hermes/profiles/haley \
  --skill smartnpc-proactive-visit \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Haley。这是一次 proactive-visit 触发 —— 系统每 15 分钟
...
严格按 smartnpc-proactive-visit 的决策流程走：
  ...
  5. 以上都通过就依次调：npc_summon(npc="Haley") →
     npc_emote(npc="Haley", kind="sparkle") →
     chat_say(speaker="Haley", text=一两句你这个角色会
