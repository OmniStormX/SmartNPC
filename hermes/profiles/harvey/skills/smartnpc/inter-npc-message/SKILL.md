...
npc_send_message(to="Penny", kind="query",
                 ...
                 reply_expected=true)
```
Harvey's own `chat_say`: "等等，我帮你问问潘妮。"

Penny wakes from `event_npc_message`, calls `npc_inbox_get`, sees
`kind="query"`. Penny checks audibility — player is in Harvey's map, not
Penny's — **not audible to Penny**. Penny only sends:
```
npc_send_message(to="Harvey", kind="reply",
                 text="图书馆下午读书会")
```
then `npc_inbox_ack(ids=[<id>])`. Penny does **not** `chat_say`.

Harvey's next turn: "潘妮说下午要去图书馆。"

### Example B — behavioral, audible

> Player → Harvey: "让阿比盖尔到我这儿来"
...
npc_send_message(to="Abigail", kind="behavioral",
                 ...
                 reply_expected=true)
```
Harvey's own `chat_say`: "好啊，我去喊阿比盖尔。"
...
```
npc_send_message(to="Harvey", kind="reply", text="OK on my way")
```
...
"Spring 5: 玩家通过 Harvey 让我去找TA，但我到的时候人已经走了，下次
