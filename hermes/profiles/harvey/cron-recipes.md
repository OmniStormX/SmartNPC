# SmartNPC Cron Templates (Harvey)
...
> snippets below are **commands you run once** in WSL with the harvey
> profile active. Re-run after a `state.db` reset.

## Required `--workdir`

Hermes cron sessions don't auto-load the profile's working directory.
Pass `--workdir /home/synchen/.hermes/profiles/harvey` so the session
sees Harvey's SOUL.md and skills.

## Recipe 1 — Proactive "3-day no contact" check-in

Fires daily at 09:07 local. Asks Harvey to check whether the player
hasn't talked to Harvey in a while, and if so, send a quiet hello
message through `chat_say` only if the player isn't busy.

```bash
hermes -p harvey cron create '7 9 * * *' \
  --name harvey-checkin-3day \
  --workdir ~/.hermes/profiles/harvey \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Harvey。检查你长期记忆，最近一次和玩家说话是什么时候。
如果超过 3 个游戏日没互动：
...
```bash
hermes -p harvey cron create '0 6 * * *' \
  --name harvey-day-mood \
  --workdir ~/.hermes/profiles/harvey \
  --skill smartnpc-memory-policy \
  '你是 Harvey。一天的开始。
调 game_get_time + game_get_weather，结合最近的玩家互动记忆，
用一句 Harvey 的语气在 memory 里记一条今日感想。不要 chat_say。'
```
...
```bash
hermes -p harvey cron create '0 22 * * *' \
  --name harvey-bedtime-sweep \
  --workdir ~/.hermes/profiles/harvey \
  --skill smartnpc-memory-policy \
  '你是 Harvey。一天结束了。
...
```bash
hermes -p harvey cron create '*/15 * * * *' \
  --name harvey-proactive-visit \
  --workdir ~/.hermes/profiles/harvey \
  --skill smartnpc-proactive-visit \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 Harvey。这是一次 proactive-visit 触发 —— 系统每 15 分钟
...
严格按 smartnpc-proactive-visit 的决策流程走：
  ...
  5. 以上都通过就依次调：npc_summon(npc="Harvey") →
     npc_emote(npc="Harvey", kind="sparkle") →
     chat_say(speaker="Harvey", text=一两句你这个角色会
...
```bash
hermes -p harvey cron list
hermes -p harvey cron status
hermes -p harvey cron remove harvey-checkin-3day
```
