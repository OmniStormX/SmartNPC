...
# Proactive visit policy — Sebastian
...
1. **`npc_summon` yourself**: `npc_summon(npc="Sebastian")`. The
...
2. **`npc_emote` sparkle bubble**: `npc_emote(npc="Sebastian",
...
3. **`chat_say` a short in-character opener**: one to two sentences,
   ...
   `speaker`: `Sebastian`. No `channel` / `group_id` — proactive
