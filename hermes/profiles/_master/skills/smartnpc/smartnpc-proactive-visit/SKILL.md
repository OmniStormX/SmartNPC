...
# Proactive visit policy — {{NPC_NAME}}
...
1. **`npc_summon` yourself**: `npc_summon(npc="{{NPC_NAME}}")`. The
...
2. **`npc_emote` sparkle bubble**: `npc_emote(npc="{{NPC_NAME}}",
...
3. **`chat_say` a short in-character opener**: one to two sentences,
   ...
   `speaker`: `{{NPC_NAME}}`. No `channel` / `group_id` — proactive
