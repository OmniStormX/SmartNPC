...
description: Optional cron module. Decide whether {{NPC_NAME}} should visit the player without being asked. Uses cooldown, availability, time window, summon/emote/chat_say.
version: 0.2.0
...
5. Visit:
   - `npc_summon(npc="{{NPC_NAME}}")`
   - `npc_emote(npc="{{NPC_NAME}}", kind="sparkle")`
   - one private `chat_say(speaker="{{NPC_NAME}}", text="...")`
