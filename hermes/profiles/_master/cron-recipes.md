...
```bash
hermes -p {{NPC_DIR}} cron create '*/15 * * * *' \
  --name {{NPC_DIR}}-proactive-visit \
  --workdir ~/.hermes/profiles/{{NPC_DIR}} \
  --skill smartnpc-proactive-visit \
  --skill smartnpc-game-tool-policy \
  --skill smartnpc-memory-policy \
  '你是 {{NPC_NAME}}。这是一次 proactive-visit 触发 —— 系统每 15 分钟
...
严格按 smartnpc-proactive-visit 的决策流程走：
  ...
  5. 以上都通过就依次调：npc_summon(npc="{{NPC_NAME}}") →
     npc_emote(npc="{{NPC_NAME}}", kind="sparkle") →
     chat_say(speaker="{{NPC_NAME}}", text=一两句你这个角色会
